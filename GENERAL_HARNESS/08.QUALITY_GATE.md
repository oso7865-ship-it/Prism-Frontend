...
| 업무 서비스와 별도 AI Runtime의 API·RAG·Tool 연동 계약/구현 | API + Security + `checklists/ai-runtime-integration-checklist.md`. 결제·DB 변경도 있으면 해당 행의 Gate를 추가 |
| 일반 DB(민감 기능 아님) | DB |
| 일반 공개 문서 | Document |
| 흐름 설명 JSON·HTML(제품 화면 설계가 아닌 문서) | Document. 실제 API·DB·보안·결제 내용을 함께 정의하면 위 해당 Gate도 누적. HTML의 상호작용 관측은 browser-qa로 증거 수집 |
...
3. 각 Gate를 독립적으로 판정한다.
   UI 기능·데이터·세션 변경에는 `checklists/ui-ux-checklist.md §1-3`을 누적한다. 단순 시각 수정에는 발동하지 않는다. 별도 AI Runtime 연동에는 위 표의 체크리스트와 [AI Runtime Handoff](AI_RUNTIME_HANDOFF_TEMPLATE.md)를 연결한다. 계약 확정, mock 검증, 실제 연동 상태는 구분한다.
...
검증 스크립트는 아래 10종이다. CI의 validate 작업은 기존 7종과 작업 기록·프로젝트 어댑터 2종, 회귀 테스트를 실행한다. `validate-package.mjs`는 ZIP export 요청 시에만 실행한다. 통계 도구와 테스트 파일은 검증 스크립트 수에 포함하지 않는다. 로컬 작업에서도 기록 범위를 선택하거나 어댑터를 변경할 때 관련 검사를 실행한다.
...
위 표의 기존 7종에 다음 3종을 포함한다.

| 스크립트 | 역할 | PASS 의미 |
|---|---|---|
| `scripts/validate-no-secrets.mjs` | Git 추적 파일의 민감 파일명·알려진 토큰 검사 | 검사한 추적 파일에서 지원 패턴 미검출 |
| `scripts/validate-work-records.mjs` | 작업 계획·Report의 범위·스킬명·헤더·최신 포인터 검사 | 실제 기록을 검사했고 확정 오류·검토 후보 없음 |
| `scripts/validate-project-adapter.mjs` | 어댑터 계약·출처 형식·문서 경로·작업 route 검사 | 선언 구조와 연결 통과. 내용·원본 커밋 인증 제외 |

어댑터 판정의 계약은 [Project Adapter](PROJECT_ADAPTER.md)가 소유한다. 선택적 어댑터 없음은 NOT_APPLICABLE이며 PASS 집계에 넣지 않는다. --require-adapter/--task 지정 시 부재는 FAIL, 기존 manifest 없는 context는 WARN(명시적 요구 시 FAIL)이다. --strict는 WARN을 비정상 종료한다. 이 검사 통과가 다른 Gate의 WARN/FAIL을 해제하지 않는다.

## 9-1. 작업 기록 검사 범위와 실행 판정

`scripts/validate-work-records.mjs`는 작업 계획·Report의 저비용 정합성 검사다. 기존 서비스 사본의 검사 개념을 일반화했으며 과거 기록을 자동 수정하지 않는다. 스킬명은 해당 프로젝트의 실제 skills 폴더 중 SKILL.md가 있는 이름과 대조한다. 스킬 인덱스와 폴더의 일치 자체는 기존 정합성 검증기가 담당한다.

- 프로젝트 루트 또는 GENERAL_HARNESS에서 실행한다. 다른 위치에서는 `--project-root`로 프로젝트 루트를 명시한다.
- 기본 경로는 프로젝트 아래 docs/work-plans와 reports이며 각각 선택 대상이다. 둘 다 기록이 없으면 PASS가 아니다.
- `--plans` 또는 `--reports`로 지정한 프로젝트 내 폴더는 필수 대상이다. 경로가 없거나 활성 기록이 0개이면 FAIL이다. 상대 경로는 프로젝트 루트 기준이다.
- README, _LATEST, 숨김 항목, archive 디렉터리는 기록 수에서 제외한다. 존재하는 최신 포인터는 별도로 검사한다. 겹치는 범위의 파일은 전체 검사 수에서 한 번만 센다.
- 경로·대상 상태·검사 개수·확인한 파일 목록을 출력한다(JSON에 전체 목록). 읽기 실패·프로젝트 밖 경로·검사 경로의 심볼릭 링크를 PASS로 처리하지 않는다.
- 명시된 미등록 스킬명, 비어 있는 Report 필수 헤더, 빈 기록, 깨진 최신 포인터는 확정 오류다. Report 헤더 검사만 명시적인 legacy 기록에 면제하며 다른 오류는 계속 검사한다.
- 위험도 대비 Gate 누락 후보, 최신 포인터 부재, 파일명 날짜 기준 후속 기록은 수동 검토 WARN이다. Gate 적용 문맥과 실제 Git 상태를 추측하지 않는다.

| 실행 상태 | 기본 종료 코드 | strict 종료 코드 | 의미 |
|---|---:|---:|---|
| PASS | 0 | 0 | 선택 범위에서 실제 기록을 검사했고 발견 사항 없음 |
| WARN | 0 | 1 | 수동 검토 후보 존재. 종료 코드 0도 PASS를 뜻하지 않음 |
| FAIL | 1 | 1 | 확정 오류 또는 입력·접근 실패 |
| NOT_CHECKED | 2 | 2 | 기본 선택 대상에 검사할 활성 기록이 없음. Gate의 새 판정 단계가 아니라 실행 상태이며 통과·정당한 N/A로 간주하지 않음 |

`--strict`는 경고가 있으면 실행을 실패로 종료하는 옵션이며 WARN의 의미 자체를 FAIL로 바꾸지 않는다. `--json`은 schemaVersion, status, exitCode, targets, checkedFiles, checkedCount, findings를 반환한다. 기존 PASS/WARN/FAIL과 위험 WARN 처리 규칙은 그대로 적용한다.

프로젝트 루트에서 실행 예:

```text
node GENERAL_HARNESS/scripts/validate-work-records.mjs --json
node GENERAL_HARNESS/scripts/validate-work-records.mjs --reports GENERAL_HARNESS/reports --strict
node --test GENERAL_HARNESS/scripts/tests/validate-work-records.test.mjs
```

최신 포인터는 로컬 Markdown 링크 또는 코드 span 경로를 지원한다. 링크는 포인터 파일 기준, 코드 span은 선택한 보고서 폴더 경로 또는 폴더 이름으로 시작하면 그 폴더를 기준으로 해석한다. 2행 표에서는 하네스 유지보수/부착 프로젝트 최신 행의 대상 셀을 읽고, '해당 프로젝트'로 위임한 행은 이 검사에서 해석하지 않는다. 외부 URL·자기 자신·archive를 활성 Report로 지정하면 FAIL이다.

필드 검사는 콜론 또는 표 형식의 한국어 메타데이터를 대상으로 하며 fenced 예제는 제외한다. 스킬은 쉼표로 구분한 canonical 이름을 사용한다. 복잡한 Markdown 전체 문법, 자유 서술, Gate 판단의 완전성, 파일명 날짜의 사실성은 보장하지 않는다. CI 실행 범위와 로컬 재현은 §9-2를 따른다.

### 9-1a. 상태 근거 검사

`06.REPORT_TEMPLATE.md §5-1`의 버전 1 상태 기록에 한해 5단계 상태·관측 시각·완료/실패/해당 없음 근거를 추가 검사한다. 완료된 로컬 검증에는 검증 대상이 필요하고 병합 완료에는 프로젝트 루트 harness.config.json의 Git 기준과 일치하는 병합 대상이 필요하다. 설정 파일이 없으면 main/dev를 추측하지 않는다. 설정 오류·중복 상태 필드·필수 상태/근거 누락은 FAIL이며, legacy도 명시한 새 상태 기록의 오류를 면제하지 않는다.

이 검사는 근거 필드와 명시된 설정의 일치만 확인한다. Git 명령·네트워크·파일 쓰기를 수행하지 않으며 실제 커밋 포함 여부·원격 PR·배포·연동의 사실성과 증거 최신성을 인증하지 않는다. 기존 서비스 문서에 새 필드가 없으면 과거 기록을 일괄 실패시키지 않는다. JSON 결과의 gitPolicy는 설정된 의도이며 관측된 Git 상태가 아니다.

## 9-2. CI 통합 범위와 재현

이 HARNESS 저장소의 validate job은 Ubuntu / Node 20에서 기존 7종 → 작업 기록 → 선택적 어댑터 → 회귀 순서로 실행한다. 기존 필수 체크 이름 validate는 유지한다. Windows / Node 24의 windows-regression job은 경로·junction·CLI 회귀를 추가 실행한다. 이 추가 job을 필수로 지정하는 원격 브랜치 보호 설정은 별도 운영 사항이며 자동 변경하지 않는다.

기존 7종은 GENERAL_HARNESS에서 실행하고 다음 명령은 저장소 루트에서 실행한다.

```sh
node GENERAL_HARNESS/scripts/validate-work-records.mjs --project-root . --plans docs/hardening --reports GENERAL_HARNESS/reports --strict --json
node GENERAL_HARNESS/scripts/validate-project-adapter.mjs --project-root . --strict --json
node --test GENERAL_HARNESS/scripts/tests/validate-work-records.test.mjs GENERAL_HARNESS/scripts/tests/validate-project-adapter.test.mjs
```

작업 기록의 두 경로는 명시적 필수 대상이다. 한쪽이 비거나 없으면 다른 기록이 있어도 FAIL이며 NOT_CHECKED(exit 2)도 CI 실패다. 과거 archive는 기록 검사의 범위 밖이며 기존 report-consistency가 역사 색인을 별도 검사한다. 어댑터는 선택적이므로 없으면 NOT_APPLICABLE(exit 0), 존재하되 manifest 없는 기존 형식이면 WARN이지만 --strict로 exit 1이다. 필요한 어댑터를 강제하려면 프로젝트 CI에 --require-adapter를 명시한다. 실패를 무시하거나 성공으로 바꾸는 continue-on-error/종료 코드 우회는 사용하지 않는다.

docs/hardening은 이 저장소의 유지보수 경로다. 다른 프로젝트에 부착할 때는 실제 계획·보고서 경로로 명시적으로 교체하고 선택적 어댑터 정책을 결정한다. 검사할 기록이 없다고 더미 기록을 만들거나 검사를 성공으로 숨기지 않는다. 로컬 통과, CI 설정 완료, 원격 CI 실행 통과는 각각 다른 사실이다. UI/AI의 의미·실제 연동은 자동으로 인증하지 않는다.
