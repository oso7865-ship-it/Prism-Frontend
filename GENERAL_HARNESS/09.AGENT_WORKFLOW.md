...
조건부 연결: 버전·API 근거가 불확실한 시점에는 `skills/documentation-lookup/SKILL.md`, 검증 시나리오가 부족하면 `skills/test-design/SKILL.md`를 사용한다. 코드 검수 요청·구체적 구조 우려에는 `skills/code-review/SKILL.md`, 흐름 문서 요청에는 `skills/universal-flow-document/SKILL.md`를 연결한다. 전 작업의 의무 순서가 아니며 기본 상시 읽기 표면을 늘리지 않는다. 실제 행동 준수 평가는 유지보수 시 `skills/skill-stocktake/SKILL.md`가 소유한다.
...
제품 기준 연결이 필요하면 [Project Adapter](PROJECT_ADAPTER.md)에 따라 선택적 어댑터를 부착한다. 원본 저장소·전체 커밋, 어댑터 버전, 공통 파일의 로컬 변경 이유, 문서 담당자와 작업별 읽기 순서를 기록한다. 기존 context만 있는 사본은 자동 이관하지 않고 수동 검토한다. 어댑터가 없는 프로젝트도 아래 초기화 절차를 그대로 사용할 수 있다.
