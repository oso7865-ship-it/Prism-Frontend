...
기능·데이터·세션 흐름을 설계할 때는 `checklists/ui-ux-checklist.md §1-3`의 해당 항목을 함께 검수한다. 구현 검증은 같은 항목에 관측 근거를 추가한다. 가짜 저장 성공, 실패의 업무 더미 대체, 인증 계약 우회는 FAIL이다. 확인하지 못한 항목은 WARN으로 기록하고, 실행 전제 조건 미비는 verification-loop의 FAIL 기준을 따른다. 단순 시각 수정에는 이 추가 검수를 발동하지 않는다.

| 판정 | 기준 |
|---|---|
| PASS | §1의 10개 필수 항목과 §1-2의 검토 기록이 모두 충족되고, 기능 흐름 변경이면 §2의 추가 검수에도 미해결 항목이 없음 |
