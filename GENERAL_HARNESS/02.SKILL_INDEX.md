...
| 17 | `documentation-lookup` | 문서 | Active | 버전별 공식 근거 조사 | API·설정·버전 호환성이 불확실할 때 |
| 18 | `test-design` | 문서 | Active | 요구사항별 검증 시나리오 설계 | 업무 규칙·권한·상태 변경이나 재발 버그의 테스트 설계가 부족할 때 |
| 19 | `code-review` | 문서 | Active | 코드·기존 테스트의 근거 기반 검수 | 코드/diff/테스트 검토 요청 또는 구체적인 구조 우려가 있을 때 |
| 20 | `universal-flow-document` | 문서 | Active | 동일 JSON 기반 차트·설명 생성 | 실행·업무 흐름의 인터랙티브 문서 생성·갱신 요청 |
| 21 | `troubleshooting-report` | 문서 | Active | 장애 원인·해결·후속 조치 기록 | 트러블슈팅 보고서 요청 또는 인계할 실패 분석이 있을 때 |
...
flowchart TD
    ...
    PLAN -.->|버전 근거 불확실| DOC[documentation-lookup]
    DOC -.->|근거 반환| PLAN
    PLAN -.->|검증 설계 부족| TEST[test-design]
    TEST --> IMPL
    B -->|코드·테스트 검토| REVIEW[code-review]
    REVIEW -->|수정 요청 범위만| IMPL
    REVIEW -->|검토 결과| QG
    B -->|흐름 문서 요청| FLOW[universal-flow-document]
    FLOW -->|HTML 관측| BQA
    FLOW --> QG
    IMPL --> VL
    ...
    AR -->|복구 후| VL
    B -->|트러블슈팅 기록 요청| TR[troubleshooting-report]
    AR -.->|실패 분석 인계 필요| TR
    TR --> R
...
| 버전별 API·설정 근거 불확실 | `documentation-lookup` | 호출한 계획·구현·진단 단계에 반환 |
| 요구사항별 검증 설계 부족 | `test-design` | `verification-loop`(구현 후 실행) |
| 코드·기존 테스트 검수 요청 | `code-review` | `test-design`(새 시나리오 필요 시) |
| 차트·단계 설명 생성·갱신 | `universal-flow-document` | `browser-qa`(HTML), `quality-gate` |
| 실제 스킬 준수 유지보수 평가 | `skill-stocktake` | 행동 평가 보조 문서 |
| 장애 원인·해결·재발 방지 기록 | `troubleshooting-report` | `agent-recovery`(진단), `verification-loop`(실행 검증) |
