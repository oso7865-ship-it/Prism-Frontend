    ...
    <div id="workspace-content" class="workspace-content" :aria-busy="busy">
      ...
      <template v-else-if="team && page === 'home'">
        ...
        <div class="home-grid">
          ...
          <section class="surface home-section home-guide"><p class="eyebrow">REVIEW WITH CONTEXT</p><h2>리뷰의 시작은<br />변경을 이해하는 것.</h2><p class="muted">PR을 선택하고 정적 분석을 실행하세요. 발견된 항목을 파일과 줄 번호로 확인할 수 있습니다.</p><div class="guide-rule"><AppIcon name="check" /><span>4개 언어 · 고정 커밋 정적 분석</span></div><div class="guide-rule"><AppIcon name="check" /><span>고정 커밋 기준의 분석 이력</span></div><RouterLink :to="{path:'/app/team',query:{team:selected}}" class="guide-team-link">함께 리뷰할 팀 관리 <AppIcon name="arrow" /></RouterLink></section>
        </div>
